...
async function main() {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    if (!service.variants || service.variants.length === 0) continue;
    ...
    const idNumber = '2' + Math.random().toString().slice(2, 15);
    const phone = '01' + Math.floor(100000000 + Math.random() * 900000000).toString();
    const price = variant.priceCents;

    const isPassport =
      (service.slug || '').includes('passport') || (service.name || '').includes('جواز');
    const isBirthCert =
      (service.slug || '').includes('birth') || (service.name || '').includes('ميلاد');
    const isDeathCert =
      (service.slug || '').includes('death') || (service.name || '').includes('وفاة');

    const orderData = {
      id: 'TEST-' + Math.random().toString(36).substring(2, 9).toUpperCase(),
      userId: adminUser.id,

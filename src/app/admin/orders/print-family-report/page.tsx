            ...
            <tbody>
              {data.map((item, index) => (
                <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="border border-black p-2 text-center font-black bg-gray-100">{item.source}</td>
                </tr>
              ))}
              {data.length === 0 && (
                <tr>
                    <td colSpan={5} className="text-center p-8 text-gray-400 font-bold">لا توجد بيانات متاحة حالياً</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <div className="h-44 w-full flex justify-center">
            {delegate?.unionCard ? (
              ...
              />
                ...
                <img src="/images/report-footer.png" className="h-32 object-contain grayscale opacity-50" alt="Footer" />
            )}
          </div>
        </div>
      </div>

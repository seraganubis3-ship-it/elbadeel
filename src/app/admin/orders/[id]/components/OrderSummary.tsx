      ...
      <div className='p-8 space-y-8'>
        ...
        {photographyFee > 0 && (
          <div className='flex justify-between items-center'>
            ...
            <span className='text-slate-900 font-black text-2xl tracking-tight'>
              +{photographyFee.toFixed(2)} <span className='text-base text-slate-400 font-bold mr-1'>جنيه</span>
            </span>
          </div>
        )}
        ...
        {order.deliveryType === 'ADDRESS' && order.deliveryFee > 0 && (
          <div className='flex justify-between items-center'>
            ...
            <span className='text-slate-900 font-black text-2xl tracking-tight'>
              +{(order.deliveryFee / 100).toFixed(2)} <span className='text-base text-slate-400 font-bold mr-1'>جنيه</span>
            </span>
          </div>
        )}
            ...
            <div className='space-y-3'>
              {(() => {
                try {
                  ...
                  return fines.map((fine: any, index: number) => fine.amount > 0 && (
                    <div key={index} className='flex justify-between items-center p-4 bg-white/70 rounded-2xl border border-red-50 text-xl font-black text-red-700 shadow-sm'>
                      <span>{fine.name}</span>
                      <span className="tracking-tight">+{((fine.amount || 0) / 100).toFixed(2)} جنيه</span>
                    </div>
                  ));
                } catch { return null; }
              })()}
        ...
        {order.servicesDetails && (() => {
          try {
            ...
            return services.map((service: any, index: number) => (
              <div key={index} className='flex justify-between items-center p-4 bg-indigo-50/30 rounded-2xl border border-indigo-50'>
                <span className='font-bold text-xl text-indigo-900'>{service.name}</span>
                <span className='font-black text-2xl text-indigo-950 tracking-tight'>+{(service.amount / 100).toFixed(2)} <span className='text-base font-bold text-indigo-400 mr-1'>جنيه</span></span>
              </div>
            ));
          } catch { return null; }
        })()}
        ...
        {order.discount && order.discount > 0 && (
          <div className='flex justify-between items-center p-4 bg-emerald-50/50 rounded-2xl border border-emerald-100 text-emerald-700 font-black text-xl'>
            ...
            <span className="tracking-tight">-{(order.discount / 100).toFixed(2)} <span className='text-base font-bold mr-1'>جنيه</span></span>
          </div>
        )}
        ...
        <div className='pt-8 border-t border-slate-100 mt-8'>
          <div className='flex justify-between items-center'>
            <div>
              <p className='text-sm font-black text-slate-400 uppercase tracking-widest mb-2 px-1'>الإجمالي النهائي</p>
            ...
            <div className='text-right'>
              ...
              <div className='text-base text-slate-500 font-black uppercase tracking-widest'>جنيه مصري</div>
            </div>
          </div>
        </div>
      </div>

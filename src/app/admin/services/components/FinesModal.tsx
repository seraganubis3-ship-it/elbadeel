...
export function FinesModal({ isOpen, onClose }: FinesModalProps) {
  ...
  const handleSave = async (id: string) => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        setFines(prev => prev.map(f => f.id === id ? { ...f, amountCents: data.fine.amountCents } : f));
        ...
        <div className='p-6 overflow-y-auto flex-1'>
          {loading ? (
            ...
            <div className='space-y-8'>
              ...
              {/* غرامات Section */}
              <div>
                <h3 className='text-lg font-bold text-gray-800 mb-4 pb-2 border-b border-gray-100'>أسعار الغرامات</h3>
                <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                  {finesOnly.map(fine => (
                    <div key={fine.id} className='bg-white border border-gray-200 rounded-xl p-4 flex items-center justify-between hover:border-indigo-200 hover:shadow-sm transition-all'>
                      <div className='font-bold text-gray-700'>{fine.name}</div>
                      <div className='flex items-center gap-2'>
                        <div className='relative w-28'>
                          <input 
                            type="number"
                            value={editValues[fine.id] === undefined ? '' : editValues[fine.id]! / 100}
                            onChange={(e) => setEditValues({ ...editValues, [fine.id]: Number(e.target.value) * 100 })}
                            ...
                            dir="ltr"
                          />
                          <span className='absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold text-gray-500'>ج</span>
                        </div>
                        <button 
                          onClick={() => handleSave(fine.id)}
                          disabled={savingId === fine.id || editValues[fine.id] === fine.amountCents}
              ...
              <div>
                <h3 className='text-lg font-bold text-gray-800 mb-4 pb-2 border-b border-gray-100'>تسعير الخدمات الإضافية الملحقة</h3>
                <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                  {servicesOnly.map(fine => (
                    <div key={fine.id} className='bg-gray-50 border border-gray-200 rounded-xl p-4 flex items-center justify-between'>
                      <div className='font-bold text-gray-700'>
                        {fine.name}
                        <div className="text-xs text-gray-500 font-normal mt-0.5">القيمة تحدد ديناميكياً أو يدوياً أثناء الطلب</div>
                      </div>

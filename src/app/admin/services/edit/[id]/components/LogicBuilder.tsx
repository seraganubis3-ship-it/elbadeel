...
interface LogicBuilderProps {
  ...
  fields: { name: string; label: string; type?: string; options?: {label:string; value:string}[] }[];
}

// Built-in system fields that refer to the Customer Profile or Order Context
const SYSTEM_FIELDS = [
  { name: 'gender', label: 'الجنس (ذكر/أنثى)', type: 'select', options: [{label:'ذكر', value:'male'}, {label:'أنثى', value:'female'}] },
  { name: 'age', label: 'العمر (سنة)', type: 'number' },
  { name: 'maritalStatus', label: 'الحالة الاجتماعية', type: 'select', options: [{label:'أعزب', value:'single'}, {label:'متزوج', value:'married'}] },
];
  ...
  useEffect(() => {
    try {
      ...
      Object.entries(data).forEach(([k, v]) => {
         ...
         if (valStr.startsWith('>')) { op = 'gt'; valStr = valStr.substring(1); }
         else if (valStr.startsWith('<')) { op = 'lt'; valStr = valStr.substring(1); }
         else if (valStr.startsWith('!')) { op = 'neq'; valStr = valStr.substring(1); }
         else if (valStr.startsWith('*')) { op = 'contains'; valStr = valStr.substring(1); } // Custom convention
          ...
          <div className='space-y-2'>
             {rules.map((rule, idx) => (
                <div key={idx} className='flex items-center gap-2 bg-white p-2 rounded-lg border border-slate-100 shadow-sm'>
                   ...
                   {(() => {
                      const selectedField = SYSTEM_FIELDS.find(f => f.name === rule.field) || fields.find(f => f.name === rule.field);

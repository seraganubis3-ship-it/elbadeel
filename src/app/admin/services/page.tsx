  ...
  useEffect(() => {
    ...
    if (query) {
      result = result.filter(
        s => s.name.includes(query) || s.slug.includes(query) || s.category?.name?.includes(query)
      );
    }
                  ...
                  filteredServices.map(service => (
                    <tr key={service.id} className='hover:bg-slate-50/80 transition-colors group'>
                      ...
                      <td className='px-6 py-4'>
                        <select
                          value={
                            !service.active ? 'INACTIVE' : service.isHidden ? 'SYSTEM' : 'PUBLIC'
                          }
                          onChange={e =>
                            updateVisibility(
                              service.id,
                              e.target.value as 'PUBLIC' | 'SYSTEM' | 'INACTIVE'
                            )
                          }
                        ...
                        >
                          ...
                          <option value='SYSTEM' className='text-gray-900 bg-white'>
                            نظام فقط
                          </option>

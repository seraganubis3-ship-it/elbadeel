      ...
      <div className='container mx-auto p-6 space-y-6'>
        ...
        <div className='grid gap-6 md:grid-cols-2 lg:grid-cols-4'>
          ...
          <Card className='border-0 shadow-xl bg-white overflow-hidden relative hover:shadow-2xl transition-shadow'>
            ...
            <CardContent>
              <div className='text-3xl font-bold text-gray-900'>
                {data.summary.totalOrders.toLocaleString('en-US')}
              </div>
              <div className='flex items-center gap-2 mt-3'>
                ...
                {data.summary.todayGrowth !== 0 && (
                  <span
                    className={`text-xs flex items-center gap-1 font-semibold ${data.summary.todayGrowth > 0 ? 'text-green-600' : 'text-red-600'}`}
                  >
                    {data.summary.todayGrowth > 0 ? (
            ...
            <CardContent>
              ...
              <div className='flex items-center gap-2 mt-3'>
                <Badge variant='success' className='text-xs'>
                  اليوم:{' '}
                  {data.summary.todayRevenue.toLocaleString('en-US', {
                    minimumFractionDigits: 0,
                    maximumFractionDigits: 0,
                  })}{' '}
                  جنيه
                ...
                {data.summary.revenueGrowth !== 0 && (
                  <span
                    className={`text-xs flex items-center gap-1 font-semibold ${data.summary.revenueGrowth > 0 ? 'text-green-600' : 'text-red-600'}`}
                  >
                    {data.summary.revenueGrowth > 0 ? (
                      <ArrowUp className='w-3 h-3' />
                    ) : (
          ...
          <Card className='border-0 shadow-xl bg-white overflow-hidden relative hover:shadow-2xl transition-shadow'>
            ...
            <CardContent>
              <div className='text-3xl font-bold text-gray-900'>
                {data.summary.totalCustomers.toLocaleString('en-US')}
              </div>
          ...
          <Card className='border-0 shadow-xl bg-white overflow-hidden relative hover:shadow-2xl transition-shadow'>
            ...
            <CardContent>
              <div className='text-3xl font-bold text-gray-900'>
                {data.summary.avgOrderValue.toLocaleString('en-US', {
                  minimumFractionDigits: 0,
                  maximumFractionDigits: 0,
                })}{' '}
                <span className='text-lg text-gray-600'>جنيه</span>
            ...
            <CardContent className='pt-6'>
              <div className='flex items-center justify-between'>
                <div>
                  ...
                  <p className='text-2xl font-bold text-gray-900 mt-1'>
                    {data.summary.totalSiteActions.toLocaleString('en-US')}
                  </p>
                </div>
                  ...
                  <ResponsiveContainer width='100%' height={300}>
                    <PieChart>
                      <Pie
                        ...
                        labelLine={false}
                        label={entry =>
                          `${statusLabels[entry.status] || entry.status || 'غير محدد'}: ${entry.count}`
                        }
                        outerRadius={100}
                        fill='#8884d8'
                        dataKey='count'
                ...
                <CardContent>
                  <div className='space-y-4'>
                    {data.topServices.map((service, index) => (
                      <div
                        key={service.id}
                        className='flex items-center justify-between p-3 rounded-lg bg-gradient-to-r from-gray-50 to-white border border-gray-100 hover:shadow-md transition-shadow'
                      >
                        ...
                        <div className='text-left'>
                          <p className='font-bold text-green-600'>
                            {service.revenue.toLocaleString('en-US')} جنيه
                          </p>
                        </div>
                      </div>
                    ))}
                    ...
                    <tbody>
                      {data.staffStats.map((staff, index) => (
                        ...
                        >
                          ...
                          <td className='p-4 text-center text-gray-700'>
                            {staff.avgOrderValue.toLocaleString('en-US', {
                              maximumFractionDigits: 0,
                            })}{' '}
                            جنيه
                          </td>
              ...
              <CardContent>
                <div className='space-y-3'>
                  {data.recentOrders.map(order => (
                    <div
                      key={order.id}
                      className='flex items-center justify-between p-4 rounded-lg bg-gradient-to-r from-gray-50 to-white border border-gray-100 hover:shadow-md transition-shadow'
                    >
                      ...
                      <div className='flex-1 text-left'>
                        <p className='font-bold text-green-600'>
                          {order.totalPrice.toLocaleString('en-US')} جنيه
                        </p>
                        <Badge
                          variant={

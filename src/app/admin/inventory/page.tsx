...
export default function InventoryPage() {
  ...
  const displayedServices = useMemo(() => {
    return services
      .filter(
        s =>
          (s.name.includes('قومي') || s.name.includes('بطاقة')) &&
          !s.name.includes('مترجم') &&
          !s.name.includes('تصحيح')
      )
  ...
  const addSerials = async () => {
    ...
    // 1. Check for duplicates within the input list itself
    const uniqueInput = Array.from(new Set(list));
    if (uniqueInput.length !== list.length) {
      alert('يوجد تكرار في الأرقام التي تحاول إضافتها (في القائمة الحالية).');
      return;
    }

    // 2. Check for duplicates against existing serials
    const existingNumbers = new Set(serials.map(s => String(s.serialNumber)));
    const duplicates = uniqueInput.filter(s => existingNumbers.has(s));

    if (duplicates.length > 0) {
      alert(`عفواً، الأرقام التالية موجودة بالفعل:\n${duplicates.join(', ')}`);
      return;
    }
          ...
          <div className='flex items-center gap-6'>
            ...
            <div>
              <h1 className='text-3xl font-black text-slate-900 tracking-tight mb-2'>
                إدارة العهدة والاستمارات
              </h1>
              <p className='text-slate-500 text-lg font-medium'>
                إدارة المخزون وربط الاستمارات بالخدمات (بطاقات الرقم القومي)
              </p>
            </div>
          </div>
          <Link
              ...
              <div className='space-y-4 max-h-[800px] overflow-y-auto pr-2 custom-scrollbar'>
                {displayedServices.length === 0 ? (
                  ...
                  displayedServices.map(svc => (
                    <div
                      key={svc.id}
                      className='bg-slate-50/50 rounded-2xl p-4 border border-slate-100 hover:border-emerald-200 transition-all'
                    >
                      ...
                      <div className='space-y-2'>
                        {svc.variants.map(v => (
                          <label
                            key={v.id}
                            className='flex items-center gap-3 p-3 bg-white rounded-xl border border-slate-100 cursor-pointer hover:border-emerald-300 transition-all group'
                          >
                            <div
                              className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-colors ${linkedVariantIds.includes(v.id) ? 'bg-emerald-500 border-emerald-500' : 'border-slate-300'}`}
                            >
                            ...
                            />
                            <span
                              className={`text-sm font-medium ${linkedVariantIds.includes(v.id) ? 'text-emerald-700' : 'text-slate-600'}`}
                            >
                              {v.name}
                            </span>
                          </label>
                        ...
                        filteredSerials.map((s: any) => (
                          <tr key={s.id} className='hover:bg-slate-50 transition-colors'>
                            ...
                            <td className='px-6 py-4'>
                              <span
                                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ${s.consumed ? 'bg-slate-100 text-slate-500' : 'bg-emerald-50 text-emerald-700'}`}
                              >
                                <span
                                  className={`w-1.5 h-1.5 rounded-full ${s.consumed ? 'bg-slate-400' : 'bg-emerald-500'}`}
                                ></span>

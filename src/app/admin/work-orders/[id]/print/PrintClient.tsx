      ...
      <div className="flex items-center justify-between border-b-2 border-black pb-4 mb-2">
        <div className="flex items-center gap-4">
             <div className="relative w-64 h-32">
                 ...
                 <img src="/images/report-header.png" alt="Logo" className="object-contain w-full h-full object-right" /> 
             </div>
        </div>
        
        <div className="text-left">
            <h2 className="text-lg font-bold text-gray-800">أمر شغل رقم</h2>
            <div className="text-xl font-black font-mono text-gray-900" dir="ltr">{workOrderNumber}</div>
        </div>
      </div>
          ...
          <table className="w-full border-collapse border border-gray-400 text-center text-sm mb-4">
            <thead>
              <tr className="bg-gray-200 print:bg-gray-300">
                <th className="border border-gray-600 px-1 py-1 font-black w-10 bg-gray-400/50">م</th>
            ...
            <tbody>
              {serviceOrders.map((order, index) => (
                <tr key={order.id} className="print:break-inside-avoid">
                  <td className="border border-gray-600 px-1 py-1 font-bold bg-gray-100">{index + 1}</td>
                  <td className="border border-gray-600 px-2 py-1 font-bold text-right">{order.customerName}</td>

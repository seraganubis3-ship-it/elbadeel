    ...
    <div className='max-w-6xl mx-auto space-y-6' dir='rtl'>
      ...
      <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100'>
        <div>
          ...
          <p className='text-slate-500 mt-1'>
            إدارة مستويات الوصول وأذونات الموظفين في لوحة التحكم.
          </p>
        </div>
        <button
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
        ...
        {[
          ...
          <div
            key={role.id}
            className='bg-slate-50/80 rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col h-full'
          >
            <div className='flex justify-between items-start mb-4'>
              <div>
                <h3 className='text-xl font-bold text-slate-800 flex items-center gap-2'>
                  {role.name}
                  <span className='px-2 py-0.5 bg-slate-200 text-slate-600 outline outline-1 outline-slate-300 rounded text-[10px] font-black'>
                    أساسي
                  </span>
                </h3>
            ...
            <div className='mt-auto pt-4 border-t border-slate-200 text-sm text-slate-500'>
              <div className='font-semibold mb-2'>
                الصلاحيات المدمجة ({role.permissions.length}):
              </div>
              <div className='flex flex-wrap gap-1.5'>
                {role.permissions.slice(0, 4).map((pid: string) => {
                  ...
                  return (
                    <span
                      key={pid}
                      className='bg-white border border-slate-200 text-slate-700 px-2 py-0.5 rounded text-xs'
                    >
                      {perm?.label || pid}
                    </span>
                  );
        ...
        {roles.map(role => (
          <div
            key={role.id}
            className='bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow p-6 flex flex-col h-full'
          >
            <div className='flex justify-between items-start mb-4'>
              <div>
          ...
          <div className='bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden'>
            <div className='p-6 border-b border-slate-100 flex justify-between items-center'>
              <h2 className='text-2xl font-black text-slate-800'>
                {editingRole ? 'تعديل الرتبة' : 'رتبة جديدة'}
              </h2>
              <button
                onClick={() => setIsModalOpen(false)}
                className='text-slate-400 hover:text-slate-600 bg-slate-50 hover:bg-slate-100 rounded-lg p-2 transition-colors'
              >
                <svg className='w-5 h-5' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
                  <path
                    strokeLinecap='round'
              ...
              <div>
                <div className='flex justify-between items-end mb-4'>
                  ...
                  <div className='flex gap-2 text-xs'>
                    <button
                      onClick={selectAll}
                      className='text-emerald-600 font-bold hover:underline py-1 px-2 hover:bg-emerald-50 rounded'
                    >
                      تحديد الكل
                    </button>
                    <button
                      onClick={deselectAll}
                      className='text-slate-500 font-bold hover:underline py-1 px-2 hover:bg-slate-100 rounded'
                    >
                      إلغاء التحديد
                    </button>
                  </div>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
                  {AVAILABLE_PERMISSIONS.map(permission => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-5 h-5 rounded flex items-center justify-center border transition-colors ${isSelected ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-slate-300 bg-white'}`}
                        >
                          {isSelected && (
                            <svg
                              className='w-3 h-3'
                        ...
                        <span
                          className={`font-semibold text-sm ${isSelected ? 'text-emerald-900' : 'text-slate-600'}`}
                        >
                          {permission.label}
                        </span>
                      </div>

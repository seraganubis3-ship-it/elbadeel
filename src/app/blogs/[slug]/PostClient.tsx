          ...
          <div className='flex flex-col gap-10'>
            ...
            >
              ...
              <nav className='flex items-center text-sm font-black text-slate-500 mb-12 uppercase tracking-widest gap-3 flex-row-reverse justify-center md:justify-start'>
                ...
                <Link href='/blogs' className='hover:text-emerald-600 transition-colors'>
                  المدونة
                </Link>
                <ChevronLeft size={16} className='rotate-180' />
              ...
              {post.tags?.length > 0 && (
                <div className='mt-20 pt-10 border-t border-slate-100 flex flex-wrap items-center gap-4 flex-row-reverse'>
                  ...
                  {post.tags.map((tag: any) => (
                    <span
                      key={tag.id}
                      className='bg-blue-50 text-blue-600 px-6 py-2.5 rounded-2xl text-sm font-bold border border-blue-100 hover:bg-blue-600 hover:text-white transition-all duration-300 cursor-default'
                    >
                      #{tag.name}
                    </span>
                  ))}
              ...
              <div className='sticky top-28 space-y-8'>
                ...
                <div className='bg-gradient-to-br from-blue-600 to-indigo-800 rounded-[2rem] p-8 text-white shadow-xl shadow-blue-900/20 text-right'>
                  ...
                  <p className='text-blue-100 mb-8 leading-relaxed'>
                    فريق البديل جاهز لمساعدتك في استخراج كافة أوراقك الرسمية بسرعة وأمان.
                  </p>
                  <Link
                    href='/services'
                    className='block w-full py-4 bg-white text-blue-600 text-center font-bold rounded-2xl hover:bg-blue-50 transition-colors shadow-lg'
                  >
                    اطلب خدمتك الآن
                  </Link>
                </div>
            ...
            <div className='grid grid-cols-1 md:grid-cols-3 gap-8'>
              {relatedPosts.map((related: any) => (
                <Link
                  ...
                  key={related.id}
                  className='group bg-white rounded-[2rem] border border-slate-100 overflow-hidden hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 flex flex-col h-full text-right'
                >

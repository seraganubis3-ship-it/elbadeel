...
export default function BlogsClient({
      ...
      const fetchPosts = async () => {
        ...
        try {
          const res = await fetch(
            `/api/blogs?page=${currentPage}&limit=${limit}&search=${searchTerm}`
          );
  ...
  return (
    <div
      className='min-h-screen bg-white flex flex-col font-sans overflow-x-hidden enlarge-text'
      dir='rtl'
    >
      <Navigation />
          ...
          <div className='container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl relative'>
            ...
            >
              ...
              <p className='text-2xl text-emerald-50/80 max-w-2xl mx-auto leading-relaxed mb-12 font-medium'>
                دليلك الشامل لاستخراج الأوراق الرسمية، متابعة القوانين الجديدة، ونصائح الخبراء
                لتسهيل معاملاتك في مصر.
              </p>
            ...
            {loading ? (
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10'>
                {[1, 2, 3, 4, 5, 6].map(i => (
                  <div
                    key={i}
                    className='bg-white/90 rounded-[3rem] h-[550px] animate-pulse border border-white/20 shadow-xl'
                  ></div>
                ))}
              </div>
            ) : posts.length === 0 ? (
              ...
              >
                ...
                <p className='text-slate-500 text-xl font-medium'>
                  جرب البحث بكلمات مختلفة أو عد لاحقاً.
                </p>
              </motion.div>
            ) : (
        ...
        <section className='py-20 bg-white border-t border-slate-100 mt-20 text-center'>
          <div className='container mx-auto px-4 max-w-4xl'>
            <h2 className='text-3xl font-bold text-slate-900 mb-4'>
              كن أول من يعرف بالقوانين الجديدة
            </h2>
            <p className='text-slate-600 mb-8 text-lg'>
              اشترك بالنشرة البريدية ليصلك كل جديد عن الخدمات الحكومية والمدونة.
            </p>
            <div className='flex flex-col sm:flex-row gap-4 max-w-lg mx-auto'>
              <input
                type='email'
                placeholder='البريد الإلكتروني'
                className='flex-grow h-14 px-6 rounded-2xl bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none text-right'
                dir='rtl'
              />
              <button className='h-14 px-10 bg-blue-600 text-white font-bold rounded-2xl hover:bg-blue-700 transition shadow-lg shadow-blue-600/20'>
                اشترك الآن
              </button>
            </div>
          </div>

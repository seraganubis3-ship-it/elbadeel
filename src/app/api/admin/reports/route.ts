...
function getPeriodRange(period: PeriodKey) {
  ...
  switch (period) {
    ...
    case "all":
    default:
      return { start: null, end: null, prevStart: null, prevEnd: null, label: "الكل", bucket: "month" as const };
  }
}
...
export async function GET(req: Request) {
  ...
  for (const k of keys) {
    bucketMap[k] = { key: k, label: labelFromKey(k, bucket), revenueCents: 0, orders: 0, completed: 0, pending: 0 };
  }

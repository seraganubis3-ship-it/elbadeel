...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!/^[a-zA-Z0-9-_]+$/.test(slug)) {
      return NextResponse.json({ error: "رابط الفئة يجب أن يحتوي على أحرف إنجليزية وأرقام وشرطات فقط" }, { status: 400 });
    }
    ...
    if (existingCategory) {
      return NextResponse.json({ error: "رابط الفئة موجود بالفعل، اختر رابطاً آخر" }, { status: 400 });
    }

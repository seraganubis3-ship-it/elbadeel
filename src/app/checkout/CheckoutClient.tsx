      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-white rounded-[2rem] shadow-xl border border-slate-100 overflow-hidden">
          ...
          <div className="flex border-b border-slate-100 bg-slate-50/50">
             {[1, 2, 3].map(s => (
                <div key={s} className={`flex-1 py-4 text-center text-sm font-bold border-b-2 transition-colors ${step >= s ? 'border-emerald-500 text-emerald-600' : 'border-transparent text-slate-400'}`}>
                   {s === 1 && 'الخدمة'}
                   {s === 2 && 'البيانات'}
                   {s === 3 && 'الحساب والدفع'}
                </div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {step === 1 && (
                <div className="space-y-6">
                  <div className="space-y-2">
                    <label className="block text-sm font-black text-slate-700">الخدمة المطلوبة</label>
                    <div className="grid grid-cols-1 gap-3 max-h-60 overflow-y-auto custom-scrollbar p-1">
                      {services.map(s => (
                        <div 
                          key={s.id}
                          onClick={() => { setSelectedServiceId(s.id); setSelectedVariantId(''); }}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {selectedService.variants.map(v => (
                          ...
                          >
                            ...
                            <span className="text-emerald-600 font-bold">{(v.priceCents / 100).toLocaleString()} ج.م</span>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              {step === 2 && (
                <div className="space-y-6 animate-in slide-in-from-right-4">
                   ...
                   {isNationalId && (
                     <div className="space-y-2">
                        <label className="text-sm font-bold text-slate-700">الرقم القومي (14 رقم)</label>
                        <input
                          required
                          type="text"
                          maxLength={14}
                ...
                <div className="space-y-6 animate-in slide-in-from-right-4">
                   <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100 mb-6">
                      ...
                      <div className="flex justify-between text-sm mb-1">
                         ...
                         <span className="font-bold">{(selectedVariant?.priceCents! / 100).toLocaleString()} ج.م</span>
                      </div>
